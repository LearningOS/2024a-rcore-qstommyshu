...
use crate::{
    ...
    task::{
        add_task,
        allocate_current_process_memory,
        current_task,
        current_user_token,
        exit_current_and_run_next,
        fetch_task_info,
        free_current_process_memory,
...
pub fn sys_mmap(start: usize, len: usize, port: usize) -> isize {
    ...
    allocate_current_process_memory(start, len, port)
}

/// YOUR JOB: Implement munmap.
pub fn sys_munmap(start: usize, len: usize) -> isize {
    trace!("kernel:pid[{}] sys_munmap NOT IMPLEMENTED", current_task().unwrap().pid.0);
    free_current_process_memory(start, len)
}
...
pub fn sys_set_priority(prio: isize) -> isize {
    ...
    if let Some(task_mut) = Arc::get_mut(&mut task) {
        task_mut.set_priority(prio)
    } else {
        -1
    }
}
