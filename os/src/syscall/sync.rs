...
pub fn sys_mutex_create(blocking: bool) -> isize {
    ...
    if
        let Some(id) = process_inner.mutex_list
            .iter()
            .enumerate()
...
pub fn sys_semaphore_create(res_count: usize) -> isize {
    ...
    let dd_enabled = process_inner.deadlock_detection_enabled;
    let id = if
        let Some(id) = process_inner.semaphore_list
            .iter()
            .enumerate()
            .find(|(_, item)| item.is_none())
            .map(|(id, _)| id)
    ...
    if dd_enabled {
        ...
        if process_inner.dd_allocation_sem[tid][sem_id] > 0 {
            ...
            debug!(
                ...
                tid,
                process_inner.dd_allocation_sem[tid],
                process_inner.dd_available_sem[sem_id]
            );
        }
    }
    drop(process_inner);
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    ...
    if dd_enabled {
        ...
        debug!("Need for Thread {}: {:?}", tid, process_inner.dd_need_sem[tid]);
        let task_num = process_inner.tasks.len();
...
pub fn sys_condvar_create() -> isize {
    ...
    let mut process_inner = process.inner_exclusive_access();
    let id = if
        let Some(id) = process_inner.condvar_list
            .iter()
            .enumerate()
            .find(|(_, item)| item.is_none())
            .map(|(id, _)| id)
